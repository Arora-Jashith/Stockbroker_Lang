...
const incomeStatementsTool = tool(
  async (input) => {
    const { ticker, period, limit } = input as { ticker: string; period?: string; limit?: number };
    try {
...
const purchaseStockTool = tool(
  (input) => {
    const { ticker, companyName, quantity, maxPurchasePrice } = input as {
      ticker?: string;
      companyName?: string;
      quantity?: number;
    ...
    return (
      `Please confirm that you want to purchase ${quantity} shares of ${ticker} at ` +
      `${
        maxPurchasePrice
          ? `$${maxPurchasePrice} per share`
          : "the current price"
      }.`
    );
  },
